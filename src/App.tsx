...
function App() {
  ...
  const calculateFirstValue = (
    ...
    setSecondValue(+(ratio * newFirstValue).toFixed(2));
  };
  ...
  const calculateSecondValue = (
    ...
    if (newSecondCurrency != secondCurrency) {
      const ratio = data.rates[newSecondCurrency] / data.rates[firstCurrency];
      setSecondCurrency(newSecondCurrency);
      setSecondValue(+(ratio * firstValue).toFixed(2));
    } else {
      const ratio = data.rates[firstCurrency] / data.rates[newSecondCurrency];
      setSecondValue(newSecondValue);
      setFirstValue(+(ratio * newSecondValue).toFixed(2));
    }
  };
  ...
  const getFormatDate = (): string => {
    ...
    if (lastupdate) {
      return new Date(lastupdate * 1000).toUTCString();
    }
    return '...';
  };
      ...
      <div className="blocks-container">
        <div className="blocks">
          ...
          <Block
            ...
            allCurrency={data && Object.keys(data.rates)}
            onChangeValue={(value: number) => {
              calculateSecondValue(value, secondCurrency);
            }}
            onChangeCurrency={(cur: string) => {
              calculateSecondValue(secondValue, cur);
            }}
          />
        </div>
